...
function App() {
  ...
  const [coin,setCoin]=useState(800000);
    ...
    <>
      ...
      <div className="flex justify-between items-center max-w-[1200px] mx-auto mt-8">
         ...
         <div className="flex  gap-6">
          <button className={`btn ${toggle&&'bg-[#e7fe29]'}  `} onClick={()=>setToggle(true)}>Available</button>
          <button className={`btn ${!toggle&&'bg-[#e7fe29]'} `}  onClick={()=>setToggle(false)}>Selected (<span>{selectedPlayers.length}</span>)</button>
         </div>
